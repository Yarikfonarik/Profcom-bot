...
# Состояние для редактирования полей мероприятия — со строгим фильтром
...
@router.callback_query(F.data.startswith("event_") & ~F.data.startswith("event_task") & ~F.data.startswith("event_merch") & ~F.data.startswith("event_shop") & ~F.data.startswith("event_info") & ~F.data.startswith("event_support") & ~F.data.startswith("event_settings"))
async def event_page(callback: CallbackQuery):
    ...
    feats = []
    if event.has_lectures: feats.append("📚")
    if event.has_tasks:    feats.append("📝")
    if event.has_shop:     feats.append("🛍")
    if not feats:          feats.append("💰")

    msg = (
        f"{status_icon} *{event.title}*\n"
        f"Видимость: {vis_text} | Функции: {' '.join(feats)}\n"
        f"💰 Стартовые баллы: {event.points}\n"
        f"👥 Участников: {participants_count} | 📚 Лекций: {len(lectures)}\n"
        f"📝 Заданий: {task_count} | 🛍 Товаров: {merch_count}\n"
    )
    if is_participant:
    ...
    buttons.append([InlineKeyboardButton(text="🆘 Обращения",               callback_data=f"event_support_admin_{event_id}")])
    buttons.append([InlineKeyboardButton(text="⚙️ Настройки",               callback_data=f"event_settings_{event_id}")])
...
async def event_settings(callback: CallbackQuery):
    ...
    vis = "🔒 Скрытое" if event.hidden else "👁 Открытое"
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text=vis, callback_data=f"toggle_hidden_{event_id}")],
...
async def toggle_feature(callback: CallbackQuery):
    ...
    event_id = int(parts[2]); feature = parts[3]
    if callback.from_user.id not in ADMIN_IDS: return await callback.answer("⛔ Нет прав", show_alert=True)
...
async def edit_event_field_start(callback: CallbackQuery, state: FSMContext):
    ...
    event_id = int(parts[2]); field = parts[3]
    prompts = {
        ...
        "image":      "🖼 Отправьте новую картинку (фото):",
    }
    await state.update_data(edit_event_id=event_id, edit_field=field)
    # Разные состояния для текста и фото
    if field == "image":
        await state.set_state(EventEditState.AWAITING_IMAGE)
    else:
        await state.set_state(EventEditState.AWAITING_VALUE)
    await callback.message.answer(
...
# СТРОГИЙ фильтр по состоянию — не перехватывает чужие сообщения
...
async def event_description(message: Message, state: FSMContext):
    ...
    await message.answer("🖼 Картинка (отправь фото или напиши «нет»):")
...
async def _show_features_choice(message, state):
    ...
    await message.answer(
        f"Выберите функции (нажми чтобы вкл/выкл):",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
...
async def do_link_task(callback: CallbackQuery):
    ...
    callback.data = f"link_tasks_{event_id}"; await link_tasks_page(callback)
...
async def do_link_merch(callback: CallbackQuery):
    ...
    await callback.answer("✅ Привязано")
    callback.data = f"link_merch_{event_id}"; await link_merch_page(callback)
...
async def confirm_close_event(callback: CallbackQuery):
    ...
    with Session() as session:
        ...
        count = session.query(EventParticipant).filter_by(event_id=event_id).count()
    await callback.message.answer(
        f"⚠️ Закрыть *{event.title}*? Баллы сгорят у {count} участников.",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ Да", callback_data=f"do_close_{event_id}"),
            InlineKeyboardButton(text="❌ Нет", callback_data=f"event_{event_id}"),
        ]]))
