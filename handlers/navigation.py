...
# ── Обнуление всех баллов ────────────────────────────────────────────────────
@router.callback_query(F.data == "reset_all_balances")
async def confirm_reset_all(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        return await callback.answer("⛔ Нет прав", show_alert=True)
    await callback.message.answer(
        "⚠️ *Обнуление баллов всех студентов*\n\nЭто действие нельзя отменить. Уверены?",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ Да", callback_data="do_reset_all_balances"),
            InlineKeyboardButton(text="❌ Отмена", callback_data="admin_panel"),
        ]])
    )


@router.callback_query(F.data == "do_reset_all_balances")
...
# ── Обнуление всех заданий ───────────────────────────────────────────────────
@router.callback_query(F.data == "reset_all_tasks")
async def confirm_reset_tasks(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        return await callback.answer("⛔ Нет прав", show_alert=True)
    await callback.message.answer(
        "⚠️ *Обнуление заданий*\n\nБудут удалены все задания и результаты их выполнения. Уверены?",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ Да", callback_data="do_reset_all_tasks"),
            InlineKeyboardButton(text="❌ Отмена", callback_data="admin_panel"),
        ]])
    )


@router.callback_query(F.data == "do_reset_all_tasks")
async def do_reset_tasks(callback: CallbackQuery):
    ...
    with Session() as session:
        ...
        session.commit()
    await callback.answer("✅ Все задания и результаты удалены!", show_alert=True)
    try:
        await callback.message.delete()
    except Exception:
        pass
...
async def confirm_reset_shop(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        return await callback.answer("⛔ Нет прав", show_alert=True)
    await callback.message.answer(
        "⚠️ *Обнуление магазина*\n\nБудут удалены все товары и история покупок. Уверены?",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ Да", callback_data="do_reset_all_shop"),
            InlineKeyboardButton(text="❌ Отмена", callback_data="admin_panel"),
        ]])
    )


@router.callback_query(F.data == "do_reset_all_shop")
async def do_reset_shop(callback: CallbackQuery):
    ...
    with Session() as session:
        ...
        session.commit()
    await callback.answer("✅ Все товары и покупки удалены!", show_alert=True)
    try:
        await callback.message.delete()
    except Exception:
        pass
